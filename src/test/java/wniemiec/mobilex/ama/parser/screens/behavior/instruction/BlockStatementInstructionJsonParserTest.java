...
class BlockStatementInstructionJsonParserTest {
    
    //-------------------------------------------------------------------------
    //		Attributes
    //-------------------------------------------------------------------------
    private BlockStatementInstructionJsonParser parser;
    ...
    @BeforeEach
    void setUp() {
        parser = BlockStatementInstructionJsonParser.getInstance();
    ...
    void testParseWithExpression() throws ParseException, IOException {
        withInstruction(buildVariableDeclaration("let", "i", buildLiteral(0)));
        doParsing();
    ...
    private void doParsing() throws ParseException, IOException {
        parsedInstruction = parser.parse(buildBlockStatement());
    }

    private JSONObject buildBlockStatement() {
        JSONObject instruction = new JSONObject();
        instruction.put("type", "BlockStatement");
        instruction.put("body", instructions);
